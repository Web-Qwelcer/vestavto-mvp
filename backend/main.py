...
# ── ТИМЧАСОВИЙ SEED ENDPOINT (буде видалено після сідування) ──────────────
...
async def seed_manager(x_admin_pass: str = Header(...)):
    """Тимчасовий endpoint — створює першого менеджера і повертає JWT."""
    ...
    async with _session_maker() as session:
        result = await session.execute(
            _select(_Manager).where(_Manager.telegram_id == 999999999)
        )

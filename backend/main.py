...
async def seed_manager(x_admin_pass: str = Header(...)):
    ...
    async with _session_maker() as session:
        r = await session.execute(_select(_Manager).where(_Manager.telegram_id == 821540583))
        mgr = r.scalar_one_or_none()
        if not mgr:
            mgr = _Manager(telegram_id=821540583, full_name="Ivan Manager",
                           role=_UserRole.MANAGER, is_active=True)
            session.add(mgr); await session.commit(); await session.refresh(mgr)
        return {"access_token": _create_token(mgr.id, mgr.telegram_id, mgr.role)}
# ─────────────────────────────────────────────────────────────────────────────
